...
\begin{equation}\label{boundary}
    \us = \partial_x^2\us = 0 \quad \text{for} \quad \stringx = 0, L.
\end{equation}
...
\SScomment[I thihk it is hard from Figure 3 to see that the b part is rattling. Maybe add the body so it is easier to see that a is attached while b is not?]
\begin{figure}[ht]
    \centering
    \begin{tikzpicture}
...
\end{equation} 
where grid function $\ugen^n_{\boldsymbol{l}}$ is a discrete approximation to $\ugen({\boldsymbol{x}},t)$ at $t=nk$ with time step $k$ (s), time index\SBcomment[Can you use "time index" in place of "sample" everywhere? Otherwise people get confused and think we're sampling something.] \SWcomment[Also above? ``..grid points in space and time indices in time." or something?] $n\geq 0$ and grid location $\boldsymbol{l}$ that depends on domain $\mathcal{D}$ of the system at hand. In the case of the string, we use $\stringx=lh_\text{s}$ with grid spacing $h_\text{s}$ (m), $\boldsymbol{l} = l \in [0,\hdots, N]$ and total number of grid points $N=L/h_\text{s}$ to yield $\us(\stringx,t) \approx \us_l^n$.
% 
% As the bridge is point-like, a definition of $\boldsymbol{l}$ is unnecessary and $\um(t) \approx \um^n$.\SBcomment[Can omit this] 

In the case of the body, we use $x=lh_\text{p}$ and $y=mh_\text{p}$ to get $\up(x, y, t) \approx \up_{(l,m)}^n$ where $\boldsymbol{l} = (l,m)$ with $l\in[0,\hdots,N_x]$ and $m\in[0,\hdots,N_y]$. \SBcomment[OK, notation getting pretty messy here. Why not just use $w$ for the plate, and $u$ for the string or something?] \SWcomment[Hmm.. should I also do that in the previous section? Then \eqref{eq:PDEform} would make less sense I guess. For the plate for example, should I then say (right above Eq. \eqref{eq:laplacian}: .. and state variable $u = w(x,y,t)$? I suppose that could work..]\SBcomment[Well, you don;t need to use $u$ in Eq. 1. Why not reserve $u$ for the string, $w$ for the plate, and use some other neutral dummy variable for Eq. 1. Like, say, $q$. It's not perfect, but it's better than using all the subscripts I think.] Here, $N_x = L_x / h_\text{p}$ and $N_y = L_y / h_\text{p}$ are the horizontal and vertical number of grid points respectively with grid spacing $h_\text{p}$ (m).
...
\SBcomment[OK, this part above is really difficult to deal with...because the operators haven't been defined yet]
...
\begin{equation}\label{eq:offset}
    \ell_\text{m}\um^n = M\delta_{tt}\um^n+M\omega_0^2(\um^n-\um_\text{off})+MR\delta_{t\cdot}\um^n
\end{equation}
...
\end{equation}
The boundary condition of the string will also change depending on the bridge offset:
\begin{equation}\label{eq:newBoundary}
    \us = \um_\text{off}\quad \text{and} \quad \partial_x^2\us=0.
\end{equation}
A schematic plot of the full system, including the offset described in Equations \eqref{eq:offset} and \eqref{eq:newBoundary} can be found in Figure \ref{fig:trombaSystem}.
\begin{figure}[h]
  \centering
...
Secondly, the bridge section shows the displacement of the bridge $\um^n$ as a green circle, the state of the body at the collision location $\up_{(l_\text{sm},m_\text{sm})}^n$, both moving vertically according to their respective displacements and finally, a static grey horizontal line denoting the offset $\um_\text{off}$, i.e., the resting position of the bridge.

Thirdly, the body section shows the state of the body $\up_{(l,m)}^n$ as a grid of rectangles changing (grey-scale) colour according to their displacement.
