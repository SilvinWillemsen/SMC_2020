...
The real-time implementation of the system has been done in C++ using the JUCE framework \cite{JUCE2020}. This section will elaborate on the algorithm and some important considerations regarding the setup of the system. 

Firstly, for more realistic and expressive sounds, we modelled the bridge -- and with that, the string -- to rest slightly above the body. Expanding $\ell_\text{m}u_\text{m}$ in \eqref{eq:massDisc} and including the offset yields
...
Secondly, in order to play different pitches on the instrument, a damping finger inspired by \cite{Willemsen2019:SMC2020} is implemented. The FDS for the string in \eqref{eq:stringDisc} can be extended to
...
\end{gather}
where $I_3$ is the cubic interpolation operator. Equation \eqref{eq:dampingFinger} essentially subtracts its own state at location $x_\text{f}$ according to the finger force $F_\text{f}$ applied. As done in \cite{Willemsen2019:SMC2020}, the fractional part used in the spreading operator ($\alpha_\text{i} = x_\text{f}/h - \text{floor}(x_\text{f}/h)$) is raised to the 7th power as it has been empirically found to scale finger position to pitch more properly in the context of FDSs. For lower values for $F_\text{f}$, energy from the bowed side of the string still transfers to the bridge side.

Realistic initialisation of $\eta$ is essential. Both $\eta_\text{sm}$ and $\eta_\text{mp}$ need to be realistically chosen at sample $n=0$, i.e. $\eta_\text{sm}^0 = 0$ and $\eta_\text{mp}^0 \leq 0$. If this is not done, the value for the potential at the previous time step $\psi^{n-1/2}$ would not make sense \textbf{(perhaps elaborate)}.
...
\begin{figure}[h]
  ...
  \caption{The virtual system in Equations \eqref{eq:fullSystemDisc} including the offset in Eq. \eqref{eq:offset}, with different important coordinates highlighted. Note that $\eta_\text{sm}$ is not shown as it is close to 0 at all times. }
...
\end{equation}
%
Lastly, through empirical testing it was decided to retrieve the output from the state of the plate right at the point of collision $(x_\text{mp},y_\text{mp})$ combined with the sound of the string at a lower volume. It can be argued that the loudest sound comes from the collision between the bridge and the body making it logical to select this point as the output position. 
...
\begin{tabular}{|l|c|c|}
    ...
    Damping & $R$ (s$^{-1}$)& $0.05$\\
    Offset & $u_\text{off}$ & $5\cdot 10^{-6}$\\\hline
...
NordicSMC, project number 86892. \MDcomment[Ducceschi's work was supported by an Early Career Fellowship from the Leverhulme Trust.]{}
\end{acknowledgments} 
