...
System \eqref{eq:fullSystem} is discretised using FDTD methods. These methods subdivide the continuous system in grid points in space and samples \SWcomment[or indices?] in time. Before going into the discretisation of the models, and collision and connection terms in the system described in \eqref{eq:fullSystem}, some finite difference operators are introduced.
...
\begin{figure}[h]
  ...
  \caption{The GUI showing the excited system with text highlighting different components. A more detailed description can be found in Section \ref{sec:GUI}. %\SWcomment[Same as in Figure 3, should I use $l_\text{f}, l_\text{sm}$, etc. instead of $x_\text{f}, x_\text{sm}$, etc.?]\SBcomment[I would just use the continuous variables for both figures...]
  }
  \label{fig:GUI}
\end{figure}
